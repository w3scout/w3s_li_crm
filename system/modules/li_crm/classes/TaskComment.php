...
class TaskComment extends \Backend
{
    ...
    public function onSubmit(\DataContainer $dc)
    {
        ...
        if ($objComment->tstamp > 0 && $this->Database->prepare("
                                                SELECT * FROM tl_li_task_comment WHERE pid=? AND tstamp > ?")
        ...
        $objTask = $this->Database->prepare("
                                        SELECT * FROM tl_li_task WHERE id=?")
    ...
    public function renderComment($row)
    {
        ...
        $objTemplate->datetime = $this->parseDate($GLOBALS['TL_CONFIG']['datimFormat'], $row['tstamp'] == '0' ? time() : $row['tstamp'] );
